...
if($_GET['mode']=="active")
{
	...
	$lid2 =mysqli_query($conn,"SELECT SUM(left_id) FROM ".$mem_name." WHERE cycle_id =".$cycle_id_final."") or die(mysqli_error());
	$lresult = mysqli_result($lid2, 0);
	$lid=$lresult;
	
	$rid2  =mysqli_query($conn,"SELECT SUM(right_id) FROM ".$mem_name." WHERE cycle_id =".$cycle_id_final."") or die(mysqli_error());
             ...
             <form method="post" action="" name="articlelist">
              <table width="95%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#D4D4C8">
                ...
                <?php 
					...
					$num_rows = mysqli_num_rows($rs_order);
...
<?php $mem_name = preg_replace("[ \t\n\r]+", "_", $mem_name);?>
					
					
<a href="<?php print "active_de_active_members.php?mode=active&mem_name=".$mem_name."&enrolled_id=".$enrolled_id; ?>">Actice</a><br>

<a href="<?php print "active_de_active_members.php?mode=inactive&mem_name=".$mem_name."&enrolled_id=".$enrolled_id;?>">inactive</a>								
					 </td>
